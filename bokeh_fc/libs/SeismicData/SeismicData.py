...
from src import SegRead as s
import numpy as np


#from libs.G2 import G2


class SeismicData():
    def __init__(self,):
        self.head = s.Heads.TraceBinHead()
        self.localName=""

        # if( attrs == None):
        #     self.open_segy(path)
        # else:
        #     self.open_from_g2(path,attrs)

    def open_segy(self,path=None,head_e=None,data_e=None,binHead_e=None,dt_e=1.0):
        """
        ...
        """
        self.ss = src.SegRead.Seg.SegReader()
        head_1,self.localName = os.path.split(path)
        if (path != None):
            self.ss.open(path)
    ...
    def open_from_g2(self,name,dict_conformity,dt_e=4.0):#TODO dt!!
        # g2 = G2(name,dict_conformity)
        # self.localName = name
        # self.head = g2.head
        # self.data=g2.data
